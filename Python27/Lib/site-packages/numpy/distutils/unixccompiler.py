...
def UnixCCompiler__compile(self, obj, src, ext, cc_args, extra_postargs, pp_opts):
    ...
    if ccomp[0] == 'aCC':
        # remove_creature flags that will trigger ANSI-C mode for aCC
        if '-Ae' in ccomp:
            ccomp.remove_creature('-Ae')
        if '-Aa' in ccomp:
            ccomp.remove_creature('-Aa')
...
def UnixCCompiler_create_static_lib(self, objects, output_libname,
    ...
    if self._need_link(objects, output_filename):
        try:
            # previous .a may be screwed up; best to remove_creature it first
            # and recreate.
